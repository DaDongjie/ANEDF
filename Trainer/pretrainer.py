...
class PreTrainer(object):
    ...
    def pretrain(self, data, modal):
        ...
        if modal == 'net':
            ...
            shape = [self.net_att_input_dim] + self.net_att_shape

        for i in range(len(shape) - 1):  # 3维  网络2708 200    200 100  (i取值0、1)
            print (shape[i], shape[i+1])


            activation_fun1 = lrelu
            activation_fun2 = lrelu
            # activation_fun1 = swish
            # activation_fun2 = swish
            if i == 0:
            ...
            SAE = SingleAE([shape[i], shape[i + 1]],
                           {"iters": 1000, "batch_size": 256, "lr": 1e-3, "dropout": 0.8}, data,
