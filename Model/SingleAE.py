...
class SingleAE(object):
    def __init__(self, shape, para, data, layer_idx, activation_fun1, activation_fun2):
        ...
        with tf.variable_scope('SAE') as scope:
            ...
            self.x_c = tf.nn.dropout(self.x, self.dropout) # x:输入， dropout:神经元被选中的概率
            # 产生服从正态分布的随机数字，参数shape表示生成张量的维度，mean是均值，stddev是标准差。
            # aaa = tf.Variable(tf.random_normal(shape=[10,10],mean=0.0, stddev=1.0))
            ...
            self.h = tf.nn.dropout(self.h, self.dropout) # 此函数是为了防止在训练中过拟合的操作，将训练输出按一定规则进行变换 保留率0.8
        ...
        self.opt = tf.train.AdamOptimizer(learning_rate=self.lr).minimize(loss=self.loss, var_list=self.var_list) # 自适应优化
    ...
    # see.run()  对变量进行初始化，执行（run）init语句
    # feed_dict参数的作用是替换图中的某个tensor的值。
    def getWb(self):
        return self.sess.run([self.W1, self.b1, self.W2, self.b2])

    def getH(self):
        return self.sess.run(self.h, feed_dict={self.x: self.data, self.dropout: 1.0}) # self.h 输出，feed_dict输入参数
    # data 是网络结构 或 属性矩阵
